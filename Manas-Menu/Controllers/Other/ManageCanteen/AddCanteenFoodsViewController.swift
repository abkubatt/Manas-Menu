...
class AddCanteenFoodsViewController: UIViewController {
    ...
    @objc private func addBtn(){
        ...
        let saveCanteenFood = Canteen(id: 0, name: nameF, type: pic3, price: price, image: imageF, amountForFree: 0)
        
        APICaller.shared.saveCanteenFood(with: saveCanteenFood) { [weak self] result in
            DispatchQueue.main.async {
                switch result{
                case .success(_):
                    let alertController = UIAlertController(title: "Success", message: "You successfully added canteen food: \(saveCanteenFood.name)", preferredStyle: .alert)
                ...
                case.failure(_):
                    let alertController = UIAlertController(title: "Error", message: "Error while adding canteen food: \(saveCanteenFood.name)", preferredStyle: .alert)
                    let okAction = UIAlertAction(title: "OK", style: .default) { (_) in
                        self?.navigationController?.popViewController(animated: true)
                    }
                    alertController.addAction(okAction)
